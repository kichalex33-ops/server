      ...
      <table>
        ...
        <tbody>${rows.map((item, idx) => `
          <tr>
            ...
            <td><button class="button ghost" onclick="detalhes(${idx})">Ver detalhes</button></td>
          </tr>
        `).join('')}</tbody>
      </table>
    </div>`;
...
function renderMapa(payload) {
  ...
  const pins = all.filter((item) => item.latitude && item.longitude).slice(0, 120).map((item, idx) => {
    ...
    return `<button class="pin" title="${valor(item, 'nome', 'codigo', 'tipo')}" style="left:${x}%;top:${y}%" onclick="detalhes(${idx})"></button>`;
  }).join('');
