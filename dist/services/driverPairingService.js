...
export function createDriverPairingService(repository) {
    async function createPairing({ motoristaId, serverUrl }) {
        if (!motoristaId)
            throw httpError(400, 'Motorista obrigatorio.');
        const data = repository.loadData();
        const motorista = data.motoristas.find((item) => String(item.id) === String(motoristaId));
        if (!motorista)
            throw httpError(404, 'Motorista nao encontrado.');
    ...
    function confirmPairing({ pairingId, pairingToken, device, platform, appVersion }) {
        if (!pairingId || !pairingToken)
            throw httpError(400, 'Pareamento e token sao obrigatorios.');
        ...
        if (!pairing)
            throw httpError(404, 'Pareamento nao encontrado.');
...
function assertPairingCanConfirm(pairing) {
    if (pairing.status === STATUS.CONFIRMADO)
        throw httpError(409, 'Pareamento ja confirmado.');
    if (pairing.status === STATUS.CANCELADO)
        throw httpError(409, 'Pareamento cancelado.');
...
function createDriverAuthPayload(motorista, serverUrl, issuedAt) {
...
function driverLoginFor(motorista) { return String(motorista.app_login || motorista.login || motorista.cpf || motorista.id); }
