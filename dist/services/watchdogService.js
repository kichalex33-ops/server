...
export function analyzeOperationalHealth(data, options = {}) {
    ...
    for (const driver of data.motoristas || []) {
        if (String(driver.status || "").toLowerCase() !== "ativo")
            continue;
...
function latestBy(items, selector) {
    ...
    for (const item of items) {
        ...
        if (!current || compareTime(item, current) > 0)
            result.set(String(key), item);
    }
    return result;
}
