...
import { analyzeOperationalHealth } from './watchdogService.js';

const collectionResponseKeys: Record<string, string> = {
  viagens: "viagens",
...
export function createLogisticService(factory: any) {
  ...
  function audit(tipo: string, payload: any = {}) {
  ...
  function productionInfra() {
    ...
    return {
      ...
      indicadores: {
        ...
        alertasAbertos: data.alertas.filter((item: any) => isOpen(item.status || "ABERTO")).length
      }
    };
  }
  ...
  function dashboardSummary() {
    ...
    const viagensHoje = data.viagens.filter((trip: any) => (trip.data_viagem || trip.dataViagem || "").slice(0, 10) === todayStr);
    ...
    return {
      ...
      viagensEmAndamento: data.viagens.filter((trip: any) => ACTIVE_TRIP_STATUSES.includes(normalizeStatus(trip.status))).length,
      motoristasAtivos: data.motoristas.filter((driver: any) => driver.status === "ativo").length,
      ultimaLocalizacao,
  ...
  async function create(collection: string, payload: any) {
    if (collection === "viagens") return createTrip(payload);
    if (collection === "motoristas") return createMotorista(payload);
    return repository.addItem(collection, payload);
  }

  function find(collection: string, id: string) {
  ...
  function listByTrip(collection: string, tripId: string) {
    const items = repository.getCollection(collection).filter((item: any) => tripMatch(item, tripId));
    return { [collectionResponseKeys[collection]]: items };
  }

  async function addGps(payload: any) {
  ...
  async function createTrip(payload: any) {
    const timestamp = nowIso();
  ...
  async function transitionTrip(tripId: string, targetStatus: string, payload: any = {}) {
  ...
  function liveMap() {
    ...
    const veiculos = data.viagens
      .filter((trip: any) => ACTIVE_TRIP_STATUSES.includes(normalizeStatus(trip.status)))
      .map((trip: any) => liveVehicle(data, trip));
    return {
  ...
  function driverLogin(payload: any) {
    const identificador = requireAny(payload, ["login", "cpf"], "login ou CPF");
    const senha = requireField(payload, "senha", "senha");
    const driver = repository.getCollection("motoristas").find((item: any) => String(item.cpf) === String(identificador) || String(item.id) === String(identificador));
    if (!driver || senha !== "OPteste 01") throw httpError(401, "Credenciais invalidas.");
  ...
  function evaluateGpsAlerts(location: any, trip: any) {
  ...
  function ensureAlert(payload: any) {
    const data = repository.loadData();
    const exists = data.alertas.some((a: any) => a.tipo === payload.tipo && tripMatch(a, payload.viagem_id) && isOpen(a.status || "ABERTO"));
    if (exists) return null;
    return repository.addItem("alertas", { status: "ABERTO", ...payload, created_at: nowIso() });
  }

  function recordEvent(payload: any) {
    return repository.addItem("eventos", { ...payload, created_at: nowIso() });
  }

  function authUserFromDriver(driver: any) {
    return {
      id: driver.id,
      nome: driver.nome,
      perfil: "motorista",
  ...
  function liveVehicle(data: any, trip: any) {
    const latest = data.localizacoes.filter((l: any) => tripMatch(l, trip.id)).at(-1);
    return {
      id: trip.id,
      status: trip.status,
...
function normalizePassengerType(type: any): string {
  const normalized = normalizeStatus(type || "PACIENTE");
  return normalized === "ACOMPANHANTE" ? "ACOMPANHANTE" : "PACIENTE";
}

function validateTripState(status: string) {
  if (!TRIP_STATES.includes(status)) throw httpError(400, `Estado invalido: ${status}`);
}

function nowIso() { return new Date().toISOString(); }

function tripMatch(item: any, tripId: string) {
