...
async function loadEmergencies() {
  ...
  list.innerHTML = (data.emergencias || []).map((item) => `
    <article class="emergency-card ${item.id === selectedEmergencyId ? "active" : ""}" data-id="${escapeHtml(item.id)}">
      ...
      <div class="meta">
        <div><span>Veiculo</span><strong>${escapeHtml(item.prefixo || item.placa || "-")}</strong></div>
        <div><span>Motorista</span><strong>${escapeHtml(item.motorista || "-")}</strong></div>
        <div><span>Viagem</span><strong>${escapeHtml(item.codigo || item.viagem_id || "-")}</strong></div>
...
function selectEmergency(item) {
  ...
  document.querySelector("#emergencyDetail").innerHTML = `
    <div class="meta">
      ...
      <div><span>GPS</span><strong>${escapeHtml(item.latitude && item.longitude ? `${item.latitude}, ${item.longitude}` : "-")}</strong></div>
      <div><span>Viagem</span><strong>${escapeHtml(item.codigo || item.viagem_id || "-")}</strong></div>
    </div>
...
document.querySelector("#protocolForm").addEventListener("submit", async (event) => {
  ...
  await api(`/emergencias/${selectedEmergencyId}/atender`, {
    method: "POST",
...
document.querySelector("#finishButton").addEventListener("click", async () => {
  if (!selectedEmergencyId) return;
  await api(`/emergencias/${selectedEmergencyId}/finalizar`, {
    method: "POST",
