(function(){
  ...
  function flush(){ if(!queue.length||!App.Http||!navigator.onLine) return; var batch=queue.splice(0,20); App.Http.request('/analytics/events',{method:'POST',body:{events:batch},skipToast:true}).catch(function(){queue=batch.concat(queue).slice(-100);}); }
