...
(function(){
  ...
  function ensureH530Assets(){
    if(!document.querySelector('link[data-h530-visual], link[href*="h530-auditoria-visual.css"]')){
      const link=document.createElement('link');
      link.rel='stylesheet';
      link.href=ASSET('assets/css/h533-correcao-final.css');
      link.dataset.h530Visual='1';
      document.head.appendChild(link);
    const fix534=document.createElement('link');fix534.rel='stylesheet';fix534.href=ASSET('assets/css/h534-correcao-final.css');fix534.dataset.h534Visual='1';document.head.appendChild(fix534);
    const fix536=document.createElement('link');fix536.rel='stylesheet';fix536.href=ASSET('assets/css/h536-contraste-tema-unificado.css');fix536.dataset.h536Visual='1';document.head.appendChild(fix536);
    }
    let final=document.querySelector('link[data-h531-visual], link[href*="h531-correcao-final.css"]');
    if(!final){
      final=document.createElement('link');
      final.rel='stylesheet';
      final.href=ASSET('assets/css/h533-correcao-final.css');
      final.dataset.h531Visual='1';
    }
    document.head.appendChild(final);
    const final534=document.createElement('link');final534.rel='stylesheet';final534.href=ASSET('assets/css/h534-correcao-final.css');final534.dataset.h534Visual='1';document.head.appendChild(final534);
    const final536=document.createElement('link');final536.rel='stylesheet';final536.href=ASSET('assets/css/h536-contraste-tema-unificado.css');final536.dataset.h536Visual='1';document.head.appendChild(final536); const trans537=document.createElement('link');trans537.rel='stylesheet';trans537.href=ASSET('assets/css/h537-transicao-tema-suave.css');trans537.dataset.h537Transition='1';document.head.appendChild(trans537); const ui540=document.createElement('link');ui540.rel='stylesheet';ui540.href=ASSET('assets/css/h540-interface-agatho-icons.css');ui540.dataset.h540Interface='1';document.head.appendChild(ui540);
  }

    if(document.querySelector('.dashboard-shell.manager-app') && !document.querySelector('link[data-h539-gestao], link[href*="h539-gestao-final.css"]')){const fix539=document.createElement('link');fix539.rel='stylesheet';fix539.href=ASSET('assets/css/h539-gestao-final.css');fix539.dataset.h539Gestao='1';document.head.appendChild(fix539);}
  ...
  function showOperator(){if(!/operador/.test(location.pathname))return; const meta=op.find(x=>x.href===file())||op.find(x=>x.href==='operador.html')||{}; if(meta.screen){document.querySelectorAll('[data-screen-panel]').forEach(p=>{const on=p.dataset.screenPanel===meta.screen; p.hidden=!on; p.classList.toggle('active',on);}); document.body.dataset.initialScreen=meta.screen;} if(meta.driverTab){document.querySelectorAll('[data-driver-panel]').forEach(b=>b.classList.toggle('active',b.dataset.driverPanel===meta.driverTab)); document.querySelectorAll('[data-driver-content]').forEach(p=>{const on=p.dataset.driverContent===meta.driverTab; p.hidden=!on; p.classList.toggle('active',on);}); document.body.dataset.initialDriverTab=meta.driverTab;} if(meta.label){const h=document.querySelector('#screenTitle'); const s=document.querySelector('#screenEyebrow'); if(h)h.textContent=meta.label; if(s)s.textContent='Painel Operador'; document.title=meta.label+' | Painel Operador';} if(meta.focus){setTimeout(()=>{const el=document.querySelector(meta.focus); if(el)el.scrollIntoView({behavior:'smooth',block:'start'});},350);}}
  function screenFromGestor(){const m=manager.find(x=>x.href===file()); return document.body?.dataset.managerScreen||m?.screen||'dashboard';}
